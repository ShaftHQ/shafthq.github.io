...
consent only. Inspect the reviewed pin, storage class, update/cleanup policy,
and deterministic fallback with `doctor local-ai-status` or
`doctor_managed_local_ai_status` as documented in
[managed local AI setup](/docs/start/local-infrastructure#inventory-defaults-and-troubleshooting).
