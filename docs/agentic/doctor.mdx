...
`doctor_suggest_fix` and `playwright_doctor_suggest_fix` cap their remediation
output at the top 5 ranked causes and tag each block with its category and
trust score, for example `LOCATOR (trust 82%)`, so an MCP client (including
the IntelliJ Assistant's `/doctor` command) can present causes in trust order
without re-deriving the ranking itself.
