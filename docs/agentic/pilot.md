...
| Grok CLI | Local stdio via `~/.grok/config.toml` (`[mcp_servers.shaft-mcp]`). Install with `--client grok`. Autobot ASK/PLAN stay read-only; AGENT mutates only with explicit approval and never defaults to `--yolo`. | Grok already loads `AGENTS.md` and `.agents/skills/`; do not add a second policy tree. |
