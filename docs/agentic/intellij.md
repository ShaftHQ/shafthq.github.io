...
0. **Prerequisites** detects the tools the setup flow depends on — Python 3,
   ...
   **Copy** button with a simple per-OS terminal command
   ...
   **Copy** button copies a Maven command that
...
1. **Upgrade project** runs a real check: the plugin reads the open project's
   ...
   version, or that the project has no SHAFT dependency yet), **Copy**
...
| Managed local AI status | "show managed local AI status" | `setup_status`, `setup_verify` list inventory; `doctor_managed_local_ai_status` reports enablement, eligibility, and the DISABLED snapshot — defaults stay off; see [managed local AI inventory](/docs/start/local-infrastructure#inventory-defaults-and-troubleshooting) |
