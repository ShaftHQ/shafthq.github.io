...
For an IDE-native beta experience in Java projects, see
[IntelliJ IDEA plugin beta](/docs/agentic/intellij). The plugin calls the same
MCP tools from an IntelliJ tool window and keeps engine, local agent, and
provider behavior outside plugin code.
