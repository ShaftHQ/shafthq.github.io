...
mobile, Capture, coding-partner planning, Doctor, and healer automation. Web GUI tools default to
...
the IDE; its first-run setup lets you choose the target agent or SHAFT IntelliJ
plugin, copy the matching installer command, run it in the IntelliJ terminal,
then checks the installation and stores the local stdio command automatically.
...
The response includes a working-set summary, ranked `reuseMatches`, existing
locator/action summaries, `missingCodeItems`, suggested MCP proof calls, a
focused verification command, evidence paths, and approval warnings. The tool
does not edit source. After review, use Capture record-at-target tools or the
calling agent's normal patch workflow to apply only the missing code.
