...
GUI tools default to SHAFT WebDriver; choose the `playwright_*` tools when the
project already uses `SHAFT.GUI.Playwright` or the user explicitly asks for
Playwright code. IntelliJ workflows and MCP clients expose the WebDriver and
Playwright tool families; the local Capture/Doctor CLI commands are listed
below. It is
...
|---|---|
| MCP accessibility snapshots and browser actions | `browser_get_page_dom`, `browser_open_intent`, `playwright_browser_get_page_dom`, `element_*`, `playwright_element_*`, and semantic click/type tools. |
...
- deterministic Playwright TestNG generation through
  ...
  when the target repository uses `SHAFT.GUI.Playwright`, plus local Capture CLI
  generation through `capture generate --backend playwright`;
