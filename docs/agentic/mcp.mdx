...
MCP tools from an IntelliJ tool window, includes Ask, Plan, and Agent mode
switching in the Assistant composer, scopes code-writing prompts to the active
editor file, supports cancel-to-kill termination, keeps the tested command list
visible beside the command dropdown, and keeps engine, local agent, and provider
behavior outside plugin code.
...
Use manual configuration only when the installer cannot update your MCP client.
You need Java 25, the selected client, the thin `shaft-mcp` JAR, its runtime
dependencies, a writable runtime workspace, and absolute paths for both the Java
executable and an argfile.
