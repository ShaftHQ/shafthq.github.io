...
- managed local AI inventory through `doctor_managed_local_ai_status` and
  `setup_status`; defaults stay off, consent is local-only, and the reviewed
  pin, storage class, update/cleanup policy, and deterministic fallback are
  documented in
