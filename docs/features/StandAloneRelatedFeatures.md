...
### 5. Local Execution (BrowserStack):

The main idea of using browser stack local is to enable us to deal with the system environments that could not be open without VPN because it will not be available too to deal with them via remote browser stack. So we browsr stak local software is routing the connections from our own connections. So accordingly, we can open our system environments on the remote devices, machines, browsers,... that we want to execute on. 

### 5.1. To configure local execution on browser stack manuall,there are some steps that we have to follow to be able to deal with browser stack local:
...
### 5.2. Confguration of local execution on browser stack using SHAFT:
...
### 5.1. To execute in web platform:
...
### 5.2. To execute in mobile platform:
