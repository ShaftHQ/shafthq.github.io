...
The `LOCAL_AI` managed provider is tracked by
[engine PR #4921](https://github.com/ShaftHQ/SHAFT_ENGINE/pull/4921). Use this
workflow with a source build until a containing SHAFT release is published.
:::
...
Clean also clears the active/previous activation record before removing current
reviewed artifacts. Any retained older files therefore stop being an approved
rollback candidate. Provision and activate two different reviewed pairs again
before expecting `ROLLBACK` planning to succeed.
