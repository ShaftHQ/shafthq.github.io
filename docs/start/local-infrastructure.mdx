...
commands, and their Status/Setup diagnostics, list the reviewed pin inventory.
...
Setup status and verify diagnostics include reviewed runtime and model
identity, licenses, provenance, artifact bytes, resource floors, update,
cleanup, and fallback. They exclude absolute cache paths, filenames,
checksums, executable names, and exact host RAM, CPU, and free-disk values.
...
`setup status` and `setup verify` list the reviewed pin even when the cache is
missing or the feature is still disabled:
...
Use the same inventory from each setup surface:
