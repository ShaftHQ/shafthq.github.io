...
The IntelliJ plugin's first-run setup walks through the same steps in its
tool window: upgrade the project, pick an agent (starts unselected; Codex is
never the default), copy the route-specific SHAFT Agentic Tools command, check
the agent connection, then check the tools installation. Every
command-transfer action is labeled **Copy**. The plugin opens a terminal with
copied setup commands pre-typed, and you press Enter to run them. The plugin
never executes the installer.

![SHAFT IntelliJ Assistant first-run setup wizard with Prerequisites marked Done, Choose agent expanded on Select an option, then Setup SHAFT Tools and Skills, Check agent connection, and Check SHAFT agentic tools installation](/img/agentic/intellij-plugin-mcp-setup.png)

Once **Check SHAFT agentic tools installation** passes, the wizard collapses the finished
steps and shows a **Ready** row confirming the MCP connection, workspace, and
agent, plus **Start chatting** or **Start without an agent**:

![Completed SHAFT IntelliJ Assistant setup showing Check SHAFT agentic tools installation marked Done and a Ready row with Start without an agent](/img/agentic/intellij-plugin-mcp-setup-success.png)
