...
import {EngineDependencies} from '@site/src/components/DocSnippets';
...
flowchart TD
    ...
    Existing --> Upgrade["upgrade_to_modular_shaft.py"]
    Agent --> MCP["shaft-mcp installer or manual client config"]
...
Run the upgrader from the existing project:
...
`shaft-engine`, scans for optional BrowserStack, visual, and desktop-video
modules, and runs Maven `test-compile`. If compilation fails, it restores all
changed files. An optional `OPENAI_API_KEY` enables up to three constrained
repair-and-recompile attempts before rollback.

Read the complete [automated upgrade and rollback guide](/docs/start/upgrade)
before running it on a production repository.
...
1. Connect your client from the [SHAFT MCP guide](/docs/agentic/mcp).
