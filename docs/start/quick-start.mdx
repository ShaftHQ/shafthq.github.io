...
tool window: upgrade the project, pick an agent, copy the route-specific SHAFT
MCP setup command, and check the result. Every command-transfer action is
labeled **Copy**. The plugin opens a terminal with copied setup commands
pre-typed, and you press Enter to run them:

![SHAFT IntelliJ Assistant first-run setup wizard with Prerequisites marked Done, Choose agent expanded, Codex CLI selected, and Copy setup command before Check setup](/img/agentic/intellij-plugin-mcp-setup.png)
