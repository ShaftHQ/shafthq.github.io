...
to choose an agent, install the route-specific SHAFT tools, and verify setup.
Runnable commands use **Open in Terminal** and copy the same command as a
fallback; copy-only output and links use **Copy**. The plugin never executes
the installer. Project upgrade unlocks after setup is verified.

![SHAFT IntelliJ Assistant first-run setup wizard with Prerequisites marked Done and Choose agent expanded](/img/agentic/intellij-plugin-mcp-setup.png)
