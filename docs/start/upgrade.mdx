...
The safe first command is the one in [Download and run](#download-and-run).
Review the generated diff, run your normal tests, then use the module map below
only when you need to explain why an optional module was added.
...
flowchart LR
    ...
    Scan --> SikuliX{"SikuliX dependency<br/>or SikuliActions usage?"}
...
See the [video module guide](/docs/integrations/video).
...
| SikuliX classes are missing                                                      | Add `shaft-sikulix`; Appium Windows sessions do not require it.                                                                       |
