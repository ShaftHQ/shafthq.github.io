...
3. Use native `By.xpath(...)` only as a last fallback for complex locators that
   cannot be expressed with Smart Locators or the locator builder.
4. Keep waits and retries as evidence-backed safety nets, not as a substitute
   for a stable locator.
5. Use [SHAFT Heal](/docs/agentic/heal) only after deterministic locator
