...
- Use `mobile_record_start`, `mobile_record_stop`, `mobile_replay_recording`, `mobile_recording_code_blocks`, and `mobile_record_at_target_code_blocks` to record, replay, and generate Java snippets that can be pasted into a SHAFT test or existing mobile page object. With `includeSensitiveValues=false` (the safe default), typed values are classified per field using the same deterministic privacy policy as web capture: password/token-like locators are redacted, while ordinary fields such as search boxes keep their values and remain replayable.
...
`driver.element().assertThat(...)`. `mobile_recording_code_blocks` returns the
replay method plus ranked mobile Page Object handoff blocks: locator inventory,
action sequence, and a draft Page Object. `mobile_record_at_target_code_blocks`
