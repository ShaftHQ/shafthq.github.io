...
SHAFT provides touch gesture support for mobile through `driver.touch()`:

```java title="TouchActionsExample.java"
// Tap
driver.touch().tap(locator);

// Swipe from one element to another
driver.touch().swipeToElement(sourceLocator, destinationLocator);

// Swipe in a direction until element is visible
driver.touch().swipeElementIntoView(targetLocator, TouchActions.SwipeDirection.DOWN);

// Long press
driver.touch().longTap(locator);
```
