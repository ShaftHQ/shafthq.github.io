...
Here are some common scenarios and how to configure SHAFT properties for them:
...
### Example 3: Run mobile app tests on Android emulator
...
SHAFT.Properties.mobile.set()
    ...
    .app("src/test/resources/apps/myApp.apk");
```

### Example 4: Enable maximum performance mode
...
**Note:** `maximumPerformanceMode` values:
- `0` = Disabled (default)
...
| -----------------------------------------------| ---------------- | --------------| --------------- |-------------
| cleanAllureResultsDirectoryBeforeExecution     | allure.accumulateHistory| `true`       | `true`, `false` | when true, accumulates history from previous test runs. When false, old results are deleted before the new run 
| N/A                                            | allure.cleanResultsDirectory| `true`       | `true`, `false` | clean allure results directory before execution
...
| **NEW**                                        | allure.customLogo|SHAFT Logo URL      | path of your Customized logo on local machine or url if your logo is hosted online | for changing SHAFT logo To your customized logo on the report
...
| remoteServerInstanceCreationTimeout | `1`           | (seconds)       | Timeout in seconds for remote server instance creation.                            |
...
| browserStack.seleniumVersion     | `4.38.0`      |                 | Selenium version to use on BrowserStack.                                                              |
...
| browserStack.appiumVersion       | `2.19.0`      |                 | Appium version to use on BrowserStack for mobile testing.                                             |
