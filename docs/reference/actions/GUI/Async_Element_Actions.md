...
tags: [web, element, async, performance, java25]
---

## Overview

SHAFT supports non-blocking element actions via Java virtual threads on the
project's Java 25 baseline through `driver.async().element()`. Actions queued on
the async executor run in parallel and return immediately, letting you kick off
multiple interactions concurrently and synchronize at a chosen point.
...
- [SHAFT_ENGINE source: AsyncElementActions](https://github.com/ShaftHQ/SHAFT_ENGINE/blob/main/shaft-engine/src/main/java/com/shaft/gui/element/AsyncElementActions.java)
