...
- Type action steps show a one-line preview of the typed text, with the full text and target element details stored as step parameters
...
In addition to full video recordings, SHAFT can create an animated GIF from the screenshots taken during execution. This lightweight alternative is encoded in the background, keeps the configured frame delay, and avoids applying the SHAFT watermark twice when screenshots already include it:
