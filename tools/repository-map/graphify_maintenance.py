#!/usr/bin/env python3
"""Audit or refresh this repository's local Graphify map."""
...
def require_clean_tracked_sources(root: Path) -> None:
    """Reject a refresh that could bind uncommitted tracked content to HEAD."""
    git = shutil.which("git")
    if git is None:
        raise ValueError("git is not on PATH; refresh requires a primary Git checkout")
    for arguments in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        ...
        if completed.returncode == 1:
            raise ValueError("refresh requires clean tracked sources")
        if completed.returncode != 0:
            raise ValueError("cannot inspect tracked source changes")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "audit":
            ...
            print(json.dumps(report, indent=2, sort_keys=True))
            return int(bool(report["missing_optional_parser"] or report["unexpected_parser_gap"]))
