...
import os
import shutil
import subprocess  # nosec B404 - fixed, list-form Git commands.
import sys
...
def require_primary_checkout(cwd: Path) -> None:
    """Reject linked worktrees before recording cache freshness."""
    git_executable = shutil.which("git")
    if git_executable is None:
        raise RuntimeError("git is not on PATH")
    completed = subprocess.run(  # nosec B603
...
def tracked_sources_clean(cwd: Path) -> bool:
    """Return whether staged and unstaged tracked content matches HEAD."""
    git_executable = shutil.which("git")
    if git_executable is None:
        raise RuntimeError("git is not on PATH")
    for arguments in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        completed = subprocess.run(  # nosec B603
            [git_executable, *arguments], cwd=cwd, check=False
        )
        if completed.returncode == 1:
            return False
        if completed.returncode != 0:
            raise RuntimeError("cannot inspect tracked source changes")
    return True
...
def cache_freshness(cwd: Path, graph_out: Path) -> tuple[bool, str]:
    if not tracked_sources_clean(cwd):
        return False, "stale - tracked source changes are not represented by HEAD"
    if not graph_out.is_dir() or not any(graph_out.iterdir()):
        return False, "absent - refresh from the primary checkout or fall back to rg"
    ...
    if not marker_path.is_file():
        return False, "stale - cache has no indexed revision marker"
    try:
...
def record_current_cache(cwd: Path, graph_out: Path) -> Path:
    ...
    if not (graph_out / MANIFEST_NAME).is_file():
        raise RuntimeError("Graphify manifest.json is absent; run refresh first")
    if not tracked_sources_clean(cwd):
        raise RuntimeError("recording freshness requires clean tracked sources")
    marker_path = graph_out / MARKER_NAME
...
def main(argv: list[str] | None = None, cwd: Path | None = None) -> int:
    ...
    try:
        ...
        if args.record_current:
            print(record_current_cache(working_directory, graph_out))
            return 0
        print(graph_out)
