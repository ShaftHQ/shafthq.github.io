...
modelsToTry.forEach((model, index) => {
  const label = index === 0 ? 'Primary' : index === 1 ? 'Fallback 1' : 'Fallback 2';
  console.log(`  ${index + 1}. ${label.padEnd(12)} : ${model}`);
});

console.log('\nHow it works:');
console.log('  - AutoBot will first try to use the primary model');
console.log('  - If that fails, it will automatically try the first fallback');
console.log('  - If that also fails, it will try the second fallback');
console.log('  - If all models fail, an error message is shown to the user');

console.log('\nModel Information:');
console.log('  - gemini-2.0-flash-exp : Latest experimental model (fastest, newest features)');
