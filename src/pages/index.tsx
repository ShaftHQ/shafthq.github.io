...
export default function Home(): JSX.Element {
  ...
  return <Layout title="SHAFT Engine" description="Release decisions backed by inspectable test evidence."><main data-testid="landing-main">
    <header className={styles.hero} data-testid="landing-hero"><EvidenceTrail /><div className={`container ${styles.heroLayout}`}><div className={styles.heroCopy}><div className={styles.brandRow}><div className={styles.logoPlate}><img className={styles.logo} src="/img/shaft.svg" width="92" height="92" alt="SHAFT" /></div><p className={styles.kicker} data-testid="landing-hero-kicker">SHAFT Engine: open-source Java test automation framework</p></div><h1>Release decisions backed by inspectable evidence.</h1><p className={styles.lead}>Run one Java project across web, mobile, API, database, and CLI. Keep native engine control, then connect Capture, Doctor, Heal, and MCP when they add evidence.</p><Ctas placement="hero" /><div className={styles.firstRun}><p className={styles.firstRunLabel} id="first-run-label">Your first run</p><ol className={styles.proofRail} aria-labelledby="first-run-label"><li>Generate project</li><li><code>{snippets.firstRunCommand}</code></li><li>Inspect report</li></ol><p className={styles.firstRunNote}>Adding SHAFT to an existing Maven project? <Link to="/docs/start/installation">Install SHAFT</Link></p></div></div><figure className={styles.heroFigure}><ImageViewerTrigger item={productImages[0]} onOpen={openImage} className={`${styles.evidenceMedia} ${styles.heroMedia}`} imageClassName={styles.evidenceImage} eager /><figcaption><strong>Real SHAFT execution.</strong> Open the report evidence and inspect it.</figcaption></figure></div></header>
