...
const evidence = [
  {title: 'Passed steps, attached proof', body: 'Inspect actions and captured evidence in the same run.', image: '/img/evidence/allure-passed-evidence.png', alt: 'Real SHAFT Allure report with passed status, readable steps, and an opened screenshot attachment.'},
    ...
    <main data-testid="landing-main">
      <header className={styles.hero} data-testid="landing-hero">
        ...
        <div className={`container ${styles.heroLayout}`}>
          ...
          <figure className={styles.heroFigure}><img src="/img/evidence/allure-passed-evidence.png" width="1440" height="1000" fetchPriority="high" alt="Real SHAFT Allure report with passed status, readable steps, and an opened screenshot attachment." /><figcaption>Real SHAFT evidence, rendered in Allure.</figcaption></figure>
        </div>
      </header>

      <section className={styles.section} aria-labelledby="evidence-heading" data-testid="landing-evidence"><div className="container"><Heading as="h2" id="evidence-heading">Evidence that survives scrutiny</Heading><p className={styles.intro}>A release result is more useful when its steps, attachments, diagnostics, and visual checks remain inspectable.</p><div className={styles.evidenceGrid}>{evidence.map((item, index) => <figure className={styles.evidenceCard} key={item.title}>{index === 2 ? <VisualEvidencePlate /> : <img src={item.image} width="1440" height="1000" loading="lazy" alt={item.alt} />}<figcaption><strong>{item.title}</strong><span>{item.body}</span></figcaption></figure>)}</div></div></section>
