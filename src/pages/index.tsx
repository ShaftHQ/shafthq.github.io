...
export default function Home(): JSX.Element {
  ...
  return <Layout title="SHAFT Engine" description="Release decisions backed by inspectable test evidence."><main data-testid="landing-main">
    ...
    <section className={`${styles.section} ${styles.trustSection}`} id="project-evidence" data-testid="landing-trust" aria-labelledby="trust-heading"><div className="container"><Heading as="h2" id="trust-heading">Verify the project before you adopt it</Heading><div className={styles.trustGrid}>{trustSignals.map(([title, detail, href]) => <a href={href} key={title} target="_blank" rel="noreferrer"><strong>{title}</strong><span>{detail}</span><FontAwesomeIcon icon={faArrowUpRightFromSquare} aria-hidden="true" /></a>)}</div></div></section>
    ...
    <section className={`${styles.section} ${styles.surfaceSection}`} id="testing-surfaces" data-testid="landing-surfaces" aria-labelledby="surfaces-heading"><div className="container"><Heading as="h2" id="surfaces-heading">One evidence model across five test surfaces</Heading><div className={styles.surfaceGrid}>{testSurfaces.map(([name, engine, detail]) => <article key={name}><Heading as="h3">{name}</Heading><strong>{engine}</strong><p>{detail}</p></article>)}</div></div></section>
